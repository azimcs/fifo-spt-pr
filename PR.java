...
public class PR
{
    ...
    public static void main(String argv[])
    {
        ...
        //stream = new Random(stream.nextLong()); 
        rand3 = new Random();
		...
		if(totalCustomersBeforeSimu == 0)
		{
	        ...
	        while(numberOfDepartures < totalCustomers)
	        {
	        	if(loop_1 == false) // customer already arrived, so new arrival is not needed for first loop each time
	        	{
	        		for(;i<=totalCustomers;i++)
	        		{
	        			...
	        			if(clockService[cusIndex1] > cusArival[i]) // This checks customer will go to queue or not.
	        			{
	        				...
	        				// Start sorting. The main concepts of priority is resided here. Sorting is done according to the assigned priority of the customer
	        				if(queueLength >= 2)
	        				{
	        					int m,n,temp_p;
	        	...
	        	{		
	        		...
	        		System.out.println("Customer "+cusIndex1+": Priority="+priority[cusIndex1]+", Interarrival Time="+df.format(cusInterArival[cusIndex1])+", Arrival Time="+df.format(cusArival[cusIndex1])+", Service Time="+df.format(cusService[cusIndex1])+", Waiting Time="+df.format(cusWaiting[cusIndex1]));
	            	numberOfDepartures++;
	            	cusIndex1++;
