	        	...
	        	if(loop_1 == false) // customer already arrived, so new arrival is not needed for first loop each time
	        	{
	        		for(;i<=totalCustomers;i++)
	        		{
	        			...
	        			if(clockService[cusIndex1] > cusArival[i]) // This checks customer will go to queue or not.
	        			{
	        				...
	        				// Start sorting. The main concepts of SJF is resided here. Sorting is done according to the shortest processing time.
	        				if(queueLength >= 2)
	        				{
	        					int m,n;
	        					double temp_s,temp_a;
	        					for(m = cusIndex1 + 1; m <= (cusIndex1 + queueLength-1); m++)
	        					{        						
	        						for(n = m; n <= (cusIndex1 + queueLength-1); n++)
	        						{
	        							if(cusService[m] > cusService[n+1])
	        							{
	        								temp_s = cusService[m];
	        	...
	        	if(loop_1 == true)
	        	{
	        		for(;i<=totalCustomers;i++)
	        		{
	        			...
	        			if(clockService[cusIndex1] > cusArival[i])
	        			{
	        				...
	        				// Start sorting. The main concepts of SJF. Sorting is done according to the shortest processing time.
	        				if(queueLength >= 2)
	        				{
	        					int m,n;
	        					double temp_s,temp_a;
